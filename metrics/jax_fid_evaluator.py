...
def _generate_samples(
    ...
    while num_generated < samples_per_host:
        ...
        if hasattr(encoder, "decode"):
            # Latent decoder expects NCHW float32 torch tensor on CPU.
            torch_imgs = torch.from_numpy(np.asarray(imgs, dtype=np.float32).transpose(0, 3, 1, 2))
            torch_imgs = encoder.decode(torch_imgs)
            imgs = torch_imgs.numpy().transpose(0, 2, 3, 1)  # back to NHWC
        else:
            imgs = np.clip(imgs * 127.5 + 128.0, 0, 255).astype(np.uint8)
...
def evaluate(
    ...
    is_mean, is_std = compute_inception_score(stats["logits"])
