...
def upsample2d(images, conv_weight, conv_pad):
    ...
    conv_weight = conv_weight.astype(images.dtype)
...
'''def grid_sample(images, grid, align_corners=False):
    ...
    def safe_gather(iy, ix):
        ...
        return images[n_idx, :, iy, ix] * valid  
...
class AugmentPipe:
    ...
    def __call__(self, images_list, p, key, debug_percentile=None):
        ...
        if G_inv is not I_3:
            ...
            # Reflect-pad to max possible size via index gathering
            # (jnp.pad mode='reflect' requires concrete pad_width).
            Hz = np.asarray(wavelets['sym6'], dtype=np.float32)
            Hz_pad = len(Hz) // 4
            max_pad_x = width - 1
            max_pad_y = height - 1
            '''def _reflect_idx(pad, n):
                idx = jnp.arange(-pad, n + pad)
                idx = jnp.abs(idx)
                period = max(1, 2 * (n - 1))
                idx = idx % period
                return jnp.where(idx >= n, 2 * (n - 1) - idx, idx)
            y_idx = _reflect_idx(max_pad_y, height)
            x_idx = _reflect_idx(max_pad_x, width)
            images_list = [images[:, :, y_idx][:, :, :, x_idx] for images in images_list]'''
            images_list = [jnp.pad(images, ((0,0),(0,0),(max_pad_y,max_pad_y),(max_pad_x,max_pad_x)), mode='reflect') for images in images_list]
           
            # Symmetric padding — content is centered, no origin adjustment needed.
            ...
            # Downsample and crop.
            conv_weight = jnp.tile(jnp.array(Hz[None, None, :]), (num_channels, 1, 1))
            conv_pad = (len(Hz) - 1) // 2
        ...
        if self.cutout > 0:
            ...
            for images in images_list:
                mean = jax.lax.stop_gradient(jnp.mean(images, axis=(2, 3), keepdims=True))
                std = jax.lax.stop_gradient(jnp.std(images, axis=(2, 3), keepdims=True))
