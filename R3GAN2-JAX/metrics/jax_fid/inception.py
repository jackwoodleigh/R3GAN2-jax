...
class InceptionV3(nn.Module):
    """
    InceptionV3 network with extended functionality for evaluation metrics.
    ...
    Extended to support:
    - Spatial feature extraction for sFID
    - Logits extraction for Inception Score
    - Multiple output modes for different evaluation metrics
    ...
    Attributes:
        ...
        return_spatial (bool): If True, return spatial features from Mixed_6e layer.
        return_logits (bool): If True, return raw logits for Inception Score.
        dtype (str): Data type.
    """
    ...
    def __call__(self, x, train=True, rng=jax.random.PRNGKey(0)):
        ...
        Returns:
            Always returns (pooled_features, spatial_features, logits) where:
            - pooled_features: shape [B, 2048]
            - spatial_features: shape [B, H, W, C] from Mixed_6e layer
            - logits: shape [B, num_classes] or None if include_head=False
        """
        ...
        # Save spatial features if requested
        x, spatial_features = InceptionC(
            channels_7x7=160,
            params_dict=utils.get(self.params_dict, "Mixed_6d"),
            dtype=self.dtype,
        )(x, train, features=self.return_spatial)
        x = InceptionC(
            channels_7x7=192,
        ...
        # Global average pooling to get 2048-d features
        x = jnp.mean(x, axis=(1, 2), keepdims=True)

        # Get pooled features (always returned)
        pooled_features = jnp.reshape(x, shape=(x.shape[0], -1))

        # Always compute logits if include_head is True
        logits = None
        if self.include_head:
            # Apply dropout and dense layer to get logits
            x_for_logits = pooled_features
            logits = Dense(
                features=self.num_classes_,
                params_dict={
                    "kernel": utils.get(self.params_dict, "fc")["kernel"],
                    "bias": utils.get(self.params_dict, "fc")["bias"] * 0.0,
                },
                dtype=self.dtype,
            )(x_for_logits)

        # Always return a consistent structure
        return pooled_features, spatial_features, logits
...
class InceptionC(nn.Module):
    ...
    def __call__(self, x, train=True, features=False):
        ...
        if features:
            return output, spatial_features
        else:
            return output
